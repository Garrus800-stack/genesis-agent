<p align="center">
  ...
  <br><br>
  <img src="https://img.shields.io/badge/version-7.9.33-d4a017?style=flat-square" alt="Version">
  <img src="https://img.shields.io/badge/tests-9090%20passing-4ade80?style=flat-square" alt="Tests">
  ...
  <img src="https://img.shields.io/badge/modules-419-e0e0e8?style=flat-square" alt="Modules">
  <img src="https://img.shields.io/badge/services-183-fbbf24?style=flat-square" alt="Services">
...
| **Autonomy** | pursues multi-step goals across restarts, graduates its own trust level (0–2) | single-turn responses |
...
Twelve layers with clear boundaries — star topology where every layer depends only on core/ and ports/, never on each other. The kernel is immutable. Critical safety files are hash-locked (16 of 43). Everything else is fair game for self-modification. Self-Preservation Invariants prevent safety regression during self-modification.
...
Since v7.9.33 the loop has a witness: a never-pruned change register journals every loss the cycle produces — pruned knowledge, released memories, consolidations — and listens to every fitness evaluation, readable via `/changes`.
...
**TrustLevelSystem** — Three trust levels: Supervised (everything needs approval), Autonomous (only critical actions like deploy/external-API/email need approval), Full Autonomy (only safety invariants block). MetaLearning data can suggest auto-upgrades for specific action types with >90% success over 50+ attempts.
...
| **Total** | **342** | **~106k** | **(agent source only)** |
...
| DI services | 170 manifest + 13 bootstrap = 183 at runtime |
| Late-bindings | 263 cross-phase dependency bindings (2 optional skipped) |
| Test suites | 616 files, 9090 tests (coverage gates: 80/76/78, ratchet floor 6014) |
