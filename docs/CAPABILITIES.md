...
- 447 source modules across 12 boot phases
- 185 DI services (172 manifest + 13 bootstrap)
- 9539 tests on Windows / 9538 on Linux, both green (measured v7.9.47). The single difference is `linux-sandbox`, whose eleventh case is the not-on-Linux branch — it does not run where it does not apply
- 499 events with 499 payload schemas (full parity)
- Architectural fitness: 127/130
- 25 CI audit gates — see [GATE-INVENTORY.md](GATE-INVENTORY.md) for the runtime gates
...
| **655 test files** | 9539 tests (Win baseline, v7.9.48), coverage gates: 80% lines, 76% branches, 78% functions |
