...
- [**CHANGELOG-v7.md**](CHANGELOG-v7.md) — all v7.x.x releases (2 entries)
- [**CHANGELOG-v6.md**](CHANGELOG-v6.md) — all v6.x.x releases (0 entries)
- [**CHANGELOG-v5.md**](CHANGELOG-v5.md) — all v5.x.x releases (0 entries)
- [**CHANGELOG-archive.md**](CHANGELOG-archive.md) — v0.x.x – v4.x.x (0 entries)
...
The first real life-run (two completed goals, eight idle thoughts, dream #1) yielded four precise findings — all four are healed. The sandbox read grant now resolves its root and compares on a separator boundary: a root containing spaces ("D:\Genesis Home") admits its children while the sibling "…Home2" stays out. A CODE step that returns no code field, and whose text does not look like code, no longer dies in the syntax parser — it is accepted as AMBIGUOUS text; real code (also arriving as output) keeps parsing and carries the parsat head on failure. At the continuation limit the partial result now travels honestly on the error (partialText, partialChars, continuationReason) — the pinned message stays, and nothing mistakes the fragment for finished code. And model selection has a memory: the last auto-choice rests in the soul (model-selection.json); a cold boot restores it instantly and lets the full scan run behind boot — the 9.9-second wait is gone, and without a memory, boot behaves exactly as before.

### Part B — The Nachklang (V2a, Genesis' own design)

"Das nehme ich mit" — when Genesis marks a moment itself, a new sibling of the v7.3.7 memory tools, `resonance-note`, condenses it immediately through one small structured model call into `{topic, stance, openQuestion}` and appends it to `.genesis/resonance.jsonl`. Nothing is ever written without an explicit mark. The idle mind picks it up as the PREFERRED topic source: the shared PickContext snapshot now carries the last condensates, and Reflect boosts strongly when they are present — while every other source stays exactly where it was, untouched. Tolerant by design: a missing or broken file degrades to silence, never to an error. And a small style healing (A5): announces must name the action, tool, or skill verbatim — the field run showed paraphrased announces drifting from what was then executed.
...
Eighteen idle activities produce — they reflect, plan, explore, dream, build. Not one of them lets Genesis deliberately do nothing. This release adds the nineteenth: **pause**, a legitimate choice of the IdleMind driven by the real rest need, and the first activity whose entire output is the choice itself. The trigger reads nothing but the raw rest level — the honest signal that already grows with metabolic depletion — scaling from neutral when rested to a threefold preference when exhausted; no invented time-of-day heuristics, no thresholds. The run makes no model call, uses no tools, fires no events and simulates no waiting: the idle tick is the time, and the metabolism's existing restore tick does the actual recovering. The choice costs one energy unit, the cheapest entry in the cost map by design.

What remains of a pause is one private journal line — "Ich habe geruht." with the current mood — written through the journal writer's native visibility routing via a new optional late-binding on the IdleMind, so a missing writer degrades to silence, never to failure. The short trace string feeds the standard activity pipeline, stays under the insight length floor, and pause is absent from the insight whitelist — proactive sharing is structurally excluded twice. The activity counts normally in the picker statistics, so the existing repetition penalty prevents pause loops for free, and the v7.9.12 rest-mode (the model-outage state, a condition that happens *to* Genesis) remains an explicitly different concept, untouched. One contract suite with seven pins covers registration as the nineteenth, the pure boost curve, the model-free run with its single private line, the double insight exclusion, the cost entry, and the naming-hygiene plus wiring source relations — 421 modules, 618 test files, services unchanged at 184.
...
- **`JournalWriter`** (`src/agent/memory/JournalWriter.js`) — three
  visibilities: `private` (Genesis only), `shared` (Alex sees too),
...
- **User preference parser** — German pattern `ich bin (\w+)` ("I am X") no longer captures stop words as user roles. German "ich bin oft" ("I am often") → skipped, German "ich bin Alex" → stored.
