...
> v7.6.3 — Complete listing of all deliberately blocking or observing code paths in Genesis.
...
| 12 | `check-ratchet --skip-tests`        | v7.5.x | Test count, fitness score, schema mismatches, broken links — all must stay above floor                                                                                                                                                 |

The 16-gate count quoted in CHANGELOG includes `npm test` itself plus a
`tsc --project tsconfig.ci.json --noEmit` typecheck and a `build-bundle.js
--ci` build step on the `ci:full` path; the table above lists the 12 audit
scripts proper.
