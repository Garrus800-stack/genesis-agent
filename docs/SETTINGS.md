...
per-installation UUID stored in `.genesis/.install-id` (v7.6.7+) and
...
| `.genesis/.install-id` | Per-installation UUID (v7.6.7+) — encryption key anchor; survives hostname/username changes and folder copies |
| `.genesis/.hauptstandort.json` | Hauptstandort identity stamp (v7.6.7+) — install-uuid, creation timestamp, hostname-history; foundation for v7.7+ Außenposten |
