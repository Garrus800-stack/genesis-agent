...
>
> For the live module list see `src/agent/cognitive/`. For aggregate stats see [CAPABILITIES.md](CAPABILITIES.md). For version-specific additions see [CHANGELOG.md](../CHANGELOG.md).
...
## Module additions after the original design
...
## Module 13: PreSleep — The Wake Routine's Mirror (v7.9.34)

The day gets its second bracket. PreSleep is a lifecycle sibling of the WakeUpRoutine: where wake runs at `boot:complete` with a 30-second box, PreSleep listens to the awaited `session:ending` emit (fired by the teardown orchestrator before TO_STOP and long before the instance-lock release) with a 10-second box. Inside it: a deterministic, service-tolerant snapshot — open goals with top titles, mood via the EmotionalState read API, the last journal title from a defensive tail read — and one first-person sentence, model-preferred with a bilingual template fallback mirroring the wake-side `LLM_BUDGET_FLOOR_MS` pattern; `thoughtSource` records honestly which path ran and thereby becomes a field metric. The anchor (`.genesis/continuity-anchor.json`) is a single overwritten object written through the atomic fsync-hard `StorageService.writeJSON`; the WakeUpRoutine reads it as its fourth context source with a seven-day freshness window and lets it speak in the re-entry entry only — source-pinned guardrails keep it out of every prompt builder and the identity summary. One anchor per process, never-throws, subscriptions via `applySubscriptionHelper`, started in the start sequence and stopped in TO_STOP.
