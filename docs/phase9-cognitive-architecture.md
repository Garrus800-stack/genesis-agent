...
> **Status (v7.6.9):** Phase 9 has grown from the original 6 modules (v5.8) to **35 files** in `src/agent/cognitive/`. This document covers the foundational architecture. For modules added after v5.8, see CHANGELOG.md and CAPABILITIES.md.
>
> **Concept-level Phase 9 module list** (some concepts span multiple files since v7.6.1 mixin extracts; physical filenames in `src/agent/cognitive/`):
> ExpectationEngine, SurpriseAccumulator, MentalSimulator, SelfNarrative, DreamCycle (split into DreamCycle + DreamCycleAnalysis + DreamCyclePhases since v7.5.x), CognitiveWorkspace, CognitiveHealthTracker, CognitiveEvents (event-emitter mixin), OnlineLearner (v5.3), LessonsStore (v5.3), ArchitectureReflection (v5.7), DynamicToolSynthesis (v5.7), ProjectIntelligence (v5.7), ReasoningTracer (v5.5 + `model:thinking-trace` subscription in v7.5.6), CognitiveSelfModel (v5.9.8), TaskOutcomeTracker (v5.9.7), MemoryConsolidator (v6.0.0), TaskRecorder (v6.0.0), AdaptiveStrategy + AdaptiveStrategyApply (v6.0.2, split in v7.5.x), QuickBenchmark (v6.0.2), CausalAnnotation (v7.0.9), InferenceEngine (v7.0.9), PatternMatcher (v7.0.9), StructuralAbstraction (v7.0.9), GoalSynthesizer (v7.0.9), CoreMemories (v7.3.7), ContextCollector (v7.3.7), SignificanceDetector (v7.3.7), WakeUpRoutine (v7.3.7), GateStats (v7.3.6), **SelfStatementLog (v7.5.5 + DE/EN parity in v7.5.6) + SelfStatementClassifier (v7.6.1 mixin extract)**. Frontier writers (SuspicionFrontier, LessonFrontier, EmotionalFrontier) live in `src/agent/organism/` rather than `cognitive/` since v7.x.
