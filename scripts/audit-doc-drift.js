...
function runChecks() {
  ...
  // modules-311, events-424, TSC-config_ok all stale) cannot recur.
  {
    ...
    if (readme) {
      ...
      const badgeChecks = {
        ...
        tests:      { live: '9534 passing',      label: 'badge: tests',
                      ...
                      //   count (9007). The constant had lagged the live count since the
                      //   v7.9.6 baseline of 8105; restored to the recorded run (host-aware
                      //   command handling + read-only shell guard test files included).
        ...
        schemas:    { live: '100%',              label: 'badge: schemas',
                      compare: (got, exp) => got === exp },
        services:   { live: 185,                 label: 'badge: services' }, // v7.9.36: +concernMonitor
  ...
  const TESTS_WIN = 9534; // v7.9.46 measured on Win (9480) + 12 field-fix pins
