...
function findRecordGateCalls() {
  ...
  for (const file of walk(SCAN_DIR)) {
    ...
    src = stripCommentsPreservingLines(src);
    let m;
    CALL_RE.lastIndex = 0;
...
function classifyVerdict(verdictArg) {
  ...
  // so the author either makes the call static or documents the source.
...
function main() {
  ...
  for (const c of calls) {
    const { kind, reason, value } = classifyVerdict(c.verdictArg);
