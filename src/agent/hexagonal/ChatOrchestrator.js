...
const { chatOrchestratorStream: _coStream } = require('./ChatOrchestratorStream'); // v7.9.48
...
class ChatOrchestrator {
  ...
  // v7.9.48: handleStream and its session helpers live in
  // ChatOrchestratorStream.js — mixed onto the prototype below.
    ...
    return _h._withRetry(async () => {
      ...
      try {
        ...
        if (this.nativeToolUse) {
          ...
          // v7.9.48: a model that answers with tool calls as TEXT was lost here.
          // NativeToolUse reads only the backend's structured tool_calls field
          // (Ollama: data.message.tool_calls) and hands the text back untouched
          // when that field is empty — and this early return sits in front of
          // parseToolCalls, which since v7.9.46 understands seven text formats
          // including the pipe-delimited tokens Kimi and Qwen emit. Every fix
          // made to that parser was therefore invisible on this path.
          //
          // The text is already in hand: parse it, do NOT ask the model again.
          // Falling through to the block below would cost a second model call
          // per turn — double latency, double cost.
          if (!result.toolCalls || result.toolCalls.length === 0) {
            const late = this.tools?.parseToolCalls?.(result.text);
            if (late && late.toolCalls && late.toolCalls.length > 0) {
              const _hn = /** @type {any} */ (this);
              return await _hn._processToolLoop(result.text, () => {}, message, 'general');
            }
          }
          return result.text;
        }
