...
class PromptBuilder {
  constructor({ selfModel, model, skills, knowledgeGraph, memory, anticipator, solutionAccumulator, selfOptimizer, storage }) {
    ...
    this._sectionPriority = [
      ...
      [2, 'autonomy',      700],   // v7.9.40 (B1): self-trace promoted — awakening/on-ask full block, else unchanged
  ...
  _getSourceContentBlock() {
    ...
    return [
      ...
      ``,
      `WICHTIG: Der Inhalt der angefragten Datei steht BEREITS oben zwischen`,
      `den === AUTORITATIVE QUELLE ===-Markern. Fasse ihn JETZT direkt zusammen`,
      `bzw. beantworte die Frage SOFORT daraus.`,
      `- Rufe KEIN Tool (file-search, knowledge-search o.ä.) auf, um die Datei`,
      `  zu suchen — sie liegt bereits hier vor.`,
      `- Kündige NICHTS an ("Ich lese die Datei jetzt...") und FRAGE NICHT`,
      `  nach Pfad oder Erlaubnis — antworte direkt.`,
      `- Beantworte AUSSCHLIESSLICH aus diesem Inhalt. Wenn die Antwort dort`,
      `  nicht steht, sage wörtlich: "Im Inhalt dieser Datei nicht enthalten."`,
      `  — KONFABULIERE NICHT aus Trainingsdaten oder allgemeinem Wissen.`,
      ``,
    ].join('\n');
  }
