...
class ArchitectureGraph {
  ...
  // v7.9.50: the viewport half — transform, wheel zoom, pan, fit and the
  // zoom toolbar — lives in ArchitectureGraphView.js and is mixed onto the
  // prototype below. The file stood at exactly 700 lines against a guard
  // that allows 700 and fails above it: no room for the next entry.
...
// v7.9.50: the viewport half is a sibling <script>, loaded before this file.
// In the renderer it is already a global; in Node (tests) it is required.
//
// This MUST stay outside the `typeof module` guard below. It was inside it at
// first, and the renderer — where `module` does not exist — skipped the whole
// block: the mixin never ran and the graph died with "_addZoomToolbar is not a
// function", while every Node test stayed green because Node has `module`.
// The mixin is what makes the class whole; it belongs to the class, not to the
// export.
{
  const _view = (typeof architectureGraphView !== 'undefined')
    ? architectureGraphView
    : require('./ArchitectureGraphView.js').architectureGraphView;
  Object.assign(ArchitectureGraph.prototype, _view);
}
