...
Genesis.UI.chat = (() => {
  ...
  return { addMessage, startStream, appendChunk, finishStream, send, stop };
})();
...
Genesis.UI.boot = (() => {
  ...
  async function onReady(status) {
    ...
    try {
      // Filesystem-based first-boot detection (bypasses health data timing issues)
      ...
      if (isFirst) {
        ...
        Genesis.UI.chat.addMessage('system', t('welcome.first'));
      }
      // Returning boot: empty. Genesis speaks when spoken to.
    } catch (err) {
      console.error('[UI] boot error:', err);
    }
  }
