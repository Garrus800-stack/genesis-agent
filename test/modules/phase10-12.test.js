...
// ════════════════════════════════════════════════════════════
// PHASE 12: AdaptiveMemory
// ════════════════════════════════════════════════════════════

describe('AdaptiveMemory', () => {
  const { AdaptiveMemory } = require('../../src/agent/hexagonal/AdaptiveMemory');

  it('should score memories', () => {
    const am = new AdaptiveMemory({ bus: mockBus, storage: mockStorage, eventStore: mockEventStore });
    const score = am.score({ id: 'mem1', text: 'Fixed critical bug in parser', timestamp: Date.now() });
    assert(typeof score === 'number', 'Returns number');
    assert(score > 0 && score <= 1.0, `Score in range 0-1: ${score}`);
  });

  it('should score important memories higher', () => {
    const am = new AdaptiveMemory({ bus: mockBus, storage: mockStorage, eventStore: mockEventStore });

    const importantScore = am.score(
      { id: 'mem2', text: 'Critical error: self-modification failed, learned to always snapshot first', timestamp: Date.now() },
      { surprise: 0.9, emotion: 0.8 }
    );

    const trivialScore = am.score(
      { id: 'mem3', text: 'Hello, how are you today?', timestamp: Date.now() },
      { surprise: 0.1, emotion: 0.1 }
    );

    assert(importantScore > trivialScore, `Important (${importantScore}) should be > trivial (${trivialScore})`);
  });

  it('should boost retention on access', () => {
    const am = new AdaptiveMemory({ bus: mockBus, storage: mockStorage, eventStore: mockEventStore });
    am.score({ id: 'mem4', text: 'Some fact', timestamp: Date.now() });
    const before = am.getScore('mem4');

    am.recordAccess('mem4');
    am.recordAccess('mem4');
    am.recordAccess('mem4');
    const after = am.getScore('mem4');

    assert(after >= before, `Score should not decrease after access: ${before} → ${after}`);
  });

  it('should select memories for pruning', () => {
    const am = new AdaptiveMemory({ bus: mockBus, storage: mockStorage, eventStore: mockEventStore });

    // Score some memories
    am.score({ id: 'm1', text: 'Critical error learned', timestamp: Date.now() }, { surprise: 0.9, emotion: 0.9 });
    am.score({ id: 'm2', text: 'Hello greeting', timestamp: Date.now() - 7 * 24 * 60 * 60 * 1000 }, { surprise: 0.0, emotion: 0.0 });

    const { keep, prune, compress } = am.selectForPruning([
      { id: 'm1', text: 'Critical error', timestamp: Date.now() },
      { id: 'm2', text: 'Hello greeting', timestamp: Date.now() - 7 * 24 * 60 * 60 * 1000 },
    ]);

    assert(keep.length + prune.length + compress.length === 2, 'All memories accounted for');
  });

  it('should consolidate and decay', async () => {
    const am = new AdaptiveMemory({
      bus: mockBus,
      storage: { readJSON: async () => { throw new Error(); }, writeJSON: async () => {}, delete: async () => {} },
      eventStore: mockEventStore,
    });

    // Score a memory with low retention
    am.score({ id: 'old', text: 'hi', timestamp: Date.now() - 30 * 24 * 60 * 60 * 1000 }, { surprise: 0.0, emotion: 0.0 });

    const result = await am.consolidate();
    assert(typeof result.decayed === 'number', 'Has decay count');
